...
def save_artifacts():
    # save models
    model_tsk1_opt.save_model(str(ARTIFACT_DIR / "model_tsk1_opt.json"))
    model_tsk2.save_model(str(ARTIFACT_DIR / "model_tsk2.json"))
    model_tsk3_opt.save_model(str(ARTIFACT_DIR / "model_tsk3_opt.json"))
    model_tsk4_opt.save_model(str(ARTIFACT_DIR / "model_tsk4_opt.json"))

    # save result tables
    tsk1_opt_result.to_csv(ARTIFACT_DIR / "tsk1_opt_result.csv", index=False)
    tsk2_result.to_csv(ARTIFACT_DIR / "tsk2_result.csv", index=False)
    tsk3_opt_result.to_csv(ARTIFACT_DIR / "tsk3_opt_result.csv", index=False)
    tsk4_opt_result.to_csv(ARTIFACT_DIR / "tsk4_opt_result.csv", index=False)

    # save summary
    model_summary_opt.to_csv(ARTIFACT_DIR / "model_summary_opt.csv", index=False)

    # save feature lists
    with open(ARTIFACT_DIR / "tsk1_opt_features.pkl", "wb") as f:
        pickle.dump(tsk1_opt_features, f)

    with open(ARTIFACT_DIR / "tsk2_features.pkl", "wb") as f:
        pickle.dump(tsk2_features, f)

    with open(ARTIFACT_DIR / "tsk3_features.pkl", "wb") as f:
        pickle.dump(tsk3_features, f)

    with open(ARTIFACT_DIR / "tsk4_features.pkl", "wb") as f:
        pickle.dump(tsk4_features, f)

    print("All artifacts have been saved to:", ARTIFACT_DIR)
